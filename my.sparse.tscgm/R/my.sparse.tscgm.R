my.sparse.tscgm <- function(data=data, lam1=NULL, lam2=NULL, nlambda=NULL,
                         ...
                         control=list())
{
  ...
  ################
  data.prep <-  my.pre.tscgm(xy.data=xy.data, time=time, model=model)
  xty <- data.prep$xty
  xtx <- data.prep$xtx
  yty <- data.prep$yty
  xtxt <- data.prep$xtxt
  ...
  if(is.null(lam1) | is.null(lam2) )  {
    lam.sequence <- my.lam.generate(model=model, nlambda=NULL, xty=xty, xtx=xtx, yty=yty,T=T, n=n)
  ...
  if (doNULL) {
    tmp.out = my.compute.sparse.tscgm(penalty=penalty, T=T, n=n, p=p, q=q, xty=xty, xtx=xtx, yty=yty,
                                   xtxt=xtxt, xtx2=xtx2, yty2=yty2, lam1=lam1, lam2=lam2,
                                   optimality = "NULL", setting=setting)
  }

  else if (dobic) {
    tmp.out = my.sparse.tscgm.bic(penalty=penalty,T=T, n=n, p=p, q=q, xty=xty, xtx=xtx, yty=yty,
                               xtxt=xtxt, xtx2=xtx2, yty2=yty2, lam1=lam1, lam2=lam2,
