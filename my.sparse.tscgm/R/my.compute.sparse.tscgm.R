my.compute.sparse.tscgm <- function( penalty=c("scad","lasso"),T=T, n=n, p=p, q=q, xty=xty, xtx=xtx, yty=yty,
                                     ...
                                     setting=setting)
{
  ...
  if (penalty=="scad") {
    while(1)
    {
      ...
      if(k==1){
        g.out = glasso::glasso(s=samp.cov,rho=lam11, thr=1.0e-4, maxit=1e4, penalize.diagonal=FALSE)
        ...
        g.out1 = glasso::glasso(s=samp.cov,rho=wt, thr=1.0e-4, maxit=1e4, penalize.diagonal=FALSE,
      ...
      warmstart=1
      if(k == 1) warmstart=0
      B=my.rblasso(s=xtx, m=xtyom, om=old.om, nlam=rho2, tol=1e-5, sbols=mab,
  ...
  if(penalty=="lasso") {
    while(1)
    {
      ...
      if(k > 1){
        ...
        g.out1 = glasso::glasso(s=samp.cov,rho=lam11, thr=1.0e-4, maxit=1e4, penalize.diagonal=FALSE,
                        start="warm", w.init=old1.om.i, wi.init=old1.om)
